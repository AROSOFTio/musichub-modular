...
import { listAdminArtists, listAdminGenres, listAdminAlbums, listAdminMusicTypes, uploadSong, AdminArtist, AdminGenre, AdminAlbum, AdminMusicType } from "@/lib/api";
      ...
      <form onSubmit={handleSubmit} className="grid gap-6 lg:grid-cols-2">
        <div className="space-y-4 rounded-2xl border border-slate-200 bg-white p-6">
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            <select name="artistId" required disabled={!artists.length} defaultValue="" className="h-10 w-full rounded-xl border border-slate-200 px-3 text-sm outline-none focus:border-violet-400">
              <option value="" disabled>Select artist</option>
              {artists.map((a) => <option key={a.id} value={a.id}>{a.name}</option>)}
            </select>
            <select name="genreId" required disabled={!genres.length} defaultValue="" className="h-10 w-full rounded-xl border border-slate-200 px-3 text-sm outline-none focus:border-violet-400">
              <option value="" disabled>Select genre</option>
              {genres.map((g) => <option key={g.id} value={g.id}>{g.name}</option>)}
            </select>
          </div>
          <div className="grid gap-3 sm:grid-cols-2">
            <select name="albumId" defaultValue="" className="h-10 w-full rounded-xl border border-slate-200 px-3 text-sm outline-none focus:border-violet-400">
