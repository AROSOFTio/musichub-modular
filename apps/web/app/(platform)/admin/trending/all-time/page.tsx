...
import { AdminSong, listAdminAllTime } from "@/lib/api";
...
export default function AllTimePage() {
  ...
  useEffect(() => {
    ...
    listAdminAllTime(accessToken).then((d) => { setSongs(d); setLoading(false); }).catch(() => setLoading(false));
  }, [accessToken]);

  return (
    <div className="space-y-6">
      <AdminPageHeader title="All Time" description="All-time most played and downloaded songs." breadcrumb={[{ label: "Admin" }, { label: "Trending" }, { label: "All Time" }]} />
      {loading ? (
        <div className="space-y-2">{Array.from({ length: 10 }).map((_, i) => <div key={i} className="h-14 animate-pulse rounded-xl bg-slate-100" />)}</div>
      ) : songs.length === 0 ? (
        <AdminEmptyState icon={TrendingUp} title="No data yet" description="Stats appear here as songs get played." />
      ) : (
            ...
            <tbody className="divide-y divide-slate-100">
              {songs.map((song, i) => (
                <tr key={song.id} className="hover:bg-slate-50">
                  ...
                  <td className="px-4 py-3 text-slate-400">{song.genre.name}</td>
