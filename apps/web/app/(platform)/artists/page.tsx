...
export default function ArtistsPage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-extrabold text-[var(--foreground)]">Artists</h1>
        <p className="mt-1 text-sm text-[var(--muted)]">Discover all artists on Musichub</p>
      </div>

      {isLoading ? (
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
          {Array.from({ length: 8 }).map((_, i) => (
            <div key={i} className="h-44 animate-pulse rounded-3xl bg-violet-100/70" />
          ))}
        </div>
      ) : artists.length === 0 ? (
          ...
          {artists.map((artist) => {
            ...
            return (
              <Link
                ...
                className="group flex flex-col items-center gap-4 rounded-3xl border border-borderSoft bg-[var(--card-bg)] p-6 text-center transition hover:border-violet-200 hover:shadow-card"
              >
                ...
                <div className="w-full">
                  <h2 className="font-bold text-[var(--foreground)] group-hover:text-violet-700">{artist.name}</h2>
                  {artist.verified ? (
                  ...
                  <div className="mt-4 grid w-full grid-cols-3 gap-2 text-center">
                    <div className="rounded-2xl bg-[var(--surface)] px-2 py-2">
                      <Music2 className="mx-auto h-4 w-4 text-violet-500" />
                      <p className="mt-1 text-sm font-bold text-[var(--foreground)]">{formatCount(totalSongs)}</p>
                      <p className="text-[11px] text-[var(--muted)]">Songs</p>
                    </div>
                    <div className="rounded-2xl bg-[var(--surface)] px-2 py-2">
                      <Headphones className="mx-auto h-4 w-4 text-violet-500" />
                      <p className="mt-1 text-sm font-bold text-[var(--foreground)]">{formatCount(totalPlays)}</p>
                      <p className="text-[11px] text-[var(--muted)]">Plays</p>
                    </div>
                    <div className="rounded-2xl bg-[var(--surface)] px-2 py-2">
                      <Download className="mx-auto h-4 w-4 text-violet-500" />
                      <p className="mt-1 text-sm font-bold text-[var(--foreground)]">{formatCount(totalDownloads)}</p>
                      <p className="text-[11px] text-[var(--muted)]">Downloads</p>
                    </div>
                  </div>
                </div>
              </Link>
