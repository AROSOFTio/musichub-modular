...
function SongRow({ song, rank }: { song: CatalogSong; rank?: number }) {
  ...
  return (
    <div className="flex items-center gap-4 rounded-2xl border border-borderSoft bg-white p-3 transition-shadow hover:shadow-card">
      {rank !== undefined ? (
      ...
      <div className="min-w-0 flex-1">
        <Link
          ...
          className="block truncate font-semibold text-slate-950 hover:text-violet-700"
        >
          {song.title}
        </Link>
        <Link
          href={`/artists/${song.artist.slug}`}
          className="block truncate text-sm text-slate-500 hover:text-violet-600"
        >
          {song.artist.name}
        </Link>
      </div>
...
function ArtistCard({ artist }: { artist: CatalogArtist }) {
  return (
    <Link
      ...
      className="flex flex-col items-center gap-3 rounded-3xl border border-borderSoft bg-white p-5 text-center transition-shadow hover:shadow-card"
    >
...
function GenreChip({ genre }: { genre: CatalogGenre }) {
  return (
    <Link
      ...
      className="group flex items-center justify-between rounded-2xl border border-borderSoft bg-white px-4 py-3 transition-colors hover:border-violet-300 hover:bg-violet-50"
    >
...
export default function HomePage() {
  ...
  if (!hasLiveContent) {
    return (
      <div className="rounded-3xl border border-borderSoft bg-white p-10 text-center shadow-card">
        <h1 className="text-2xl font-bold text-slate-950">Music is coming soon</h1>
