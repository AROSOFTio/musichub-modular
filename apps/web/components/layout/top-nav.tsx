...
const desktopMenuItems = [
  ...
  { href: "/artists", label: "Artists", moduleKey: MODULE_KEYS.artists },
...
export function TopNav() {
  ...
  return (
    <header className="sticky top-0 z-30 hidden border-b border-borderSoft bg-white/95 backdrop-blur lg:block">
      <div className="flex w-full items-center justify-between gap-5 px-6 py-4">
        <Logo className="shrink-0 [&_img]:h-12" />
        <div className="min-w-0 flex-1 max-w-xl">
          {hasModule(modules, MODULE_KEYS.search) ? <SearchBox /> : null}
        </div>

        <nav className="flex min-w-0 flex-1 items-center justify-center gap-1 rounded-2xl bg-violet-50/60 px-2 py-1">
          {menuItems.map((item) => (
            <Link
              key={item.href}
              href={item.href}
              className="inline-flex items-center gap-1.5 rounded-xl px-3 py-2 text-sm font-bold text-slate-800 transition hover:bg-white hover:text-violet-700"
            >
              {item.label}
              {item.badge ? (
